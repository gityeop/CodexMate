...
final class CodexDesktopConversationActivityReaderTests: XCTestCase {
    func testLatestViewedAtByThreadIDParsesRecentThreadResumeEvents() throws {
        ...
        let latestViewedAtByThreadID = reader.latestViewedAtByThreadID(
            now: Date(timeIntervalSince1970: 1_773_195_200)
        )

        XCTAssertEqual(latestViewedAtByThreadID["thread-1"], date("2026-03-11T12:17:11.346Z"))
        XCTAssertEqual(latestViewedAtByThreadID["thread-2"], date("2026-03-11T12:09:16.169Z"))
        XCTAssertNil(latestViewedAtByThreadID["thread-3"])
    }
