...
final class CodexDesktopStateReaderTests: XCTestCase {
    ...
    func testSnapshotPreflightSkipsStaleActiveSessionWhenTerminalActivityIsNewer() throws {
        let tempDirectoryURL = FileManager.default.temporaryDirectory
            .appending(path: UUID().uuidString)
        try FileManager.default.createDirectory(at: tempDirectoryURL, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDirectoryURL) }
        ...
        let snapshot = try reader.snapshot(
            candidateSessionContexts: [
                "thread-1": ThreadSessionContext(
                    path: sessionURL.path,
                    authoritativeUpdatedAt: Date(timeIntervalSince1970: 90),
                    authoritativeTerminalActivityAt: Date(timeIntervalSince1970: 200),
                    authoritativeStatusIsPending: false,
                    authoritativeStatusIsActive: false
                )
            ]
        )
        ...
        XCTAssertEqual(snapshot.runningThreadIDs, [])
        XCTAssertTrue(snapshot.debugSummary.contains("session-preflight-stale"))
        XCTAssertFalse(snapshot.debugSummary.contains("session-active"))
    }
    ...
    func testThreadsReflectsActiveTaskFromSessionFile() throws {
        ...
        let sessionURL = tempDirectoryURL.appending(path: "thread-active.jsonl")
        try """
        {"timestamp":"2026-05-04T03:11:37.043Z","type":"event_msg","payload":{"type":"task_started","turn_id":"turn-1"}}
        """.write(to: sessionURL, atomically: true, encoding: .utf8)
        ...
        let reader = CodexDesktopStateReader(stateDatabaseURLOverride: databaseURL)
        let threads = try reader.threads(threadIDs: ["thread-active"])
